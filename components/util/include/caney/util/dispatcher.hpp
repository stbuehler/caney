...
namespace caney {
	namespace util {
		inline namespace v1 {
			...
			/* return a callable which passes all calls to callable through a dispatcher using dispatch() on the dispatcher */
			template<typename Dispatcher, typename Callable>
			...
			/* return a callable which passes all calls to callable through a dispatcher using post() on the dispatcher */
			template<typename Dispatcher, typename Callable>
