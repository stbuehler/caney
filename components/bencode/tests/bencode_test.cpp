...
BOOST_AUTO_TEST_CASE(test_big_number) {
	...
	caney::optional<caney::bencode::big_number> i = caney::bencode::parse<caney::bencode::big_number>(buf);
	BOOST_REQUIRE(i);
...
BOOST_AUTO_TEST_CASE(test_int8_t) {
	...
	caney::optional<int8_t> i = caney::bencode::parse<int8_t>(buf);
	BOOST_REQUIRE(i);
	BOOST_CHECK(buf.empty());
	BOOST_CHECK_EQUAL(i.value(), -128);
}

BOOST_AUTO_TEST_CASE(test_int_vector) {
	caney::memory::shared_const_buf buf(std::string("li0ei-1ei1ei-128ei127ee"));
	caney::optional<std::vector<int8_t>> l = caney::bencode::parse<std::vector<int8_t>>(buf);
	BOOST_REQUIRE(l);
	BOOST_CHECK(buf.empty());
